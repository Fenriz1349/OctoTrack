...
final class GitHubAuthenticationEndpointTests: XCTestCase {
    ...
    func test_authorizeURL_buildsCorrectURL() throws {
        ...
        XCTAssertEqual(queryItems?.first(where: { $0.name == "client_id" })?.value,
                       GitHubAuthenticationEndpoint.config.clientID)
        XCTAssertEqual(queryItems?.first(where: { $0.name == "redirect_uri" })?.value,
                       GitHubAuthenticationEndpoint.config.redirectURI)
